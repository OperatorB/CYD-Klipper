...
const char *rgb_options = "Red\nGreen\nBlue\nTeal\nOrange\nYellow\nPurple\nOff";
const char rgb_options_values[8][3] = {
    {120, 0, 0},
    {0, 180, 0},
    {0, 0, 120},
    {0, 180, 100},
...
static void rgb_dropdown(lv_event_t *e)
{
    lv_obj_t *dropdown = lv_event_get_target(e);
    auto selected = lv_dropdown_get_selected(dropdown);
    global_config.LEDcolors[0] = rgb_options_values[selected][0];
    global_config.LEDcolors[1] = rgb_options_values[selected][1];
    global_config.LEDcolors[2] = rgb_options_values[selected][2];
...
void settings_panel_init(lv_obj_t *panel)
{
    ...
    lv_dropdown_set_options(dropdown, rgb_options);
    lv_obj_add_event_cb(dropdown, rgb_dropdown, LV_EVENT_VALUE_CHANGED, NULL);
    for (int i = 0; i < sizeof(rgb_options_values) / sizeof(rgb_options_values[0]); i++)
    {
        if (rgb_options_values[i][0] == global_config.LEDcolors[0] &&
            rgb_options_values[i][1] == global_config.LEDcolors[1] &&
            rgb_options_values[i][2] == global_config.LEDcolors[2])
        {
            lv_dropdown_set_selected(dropdown, i);
        }
    }
    create_settings_widget("LED color", dropdown, panel);
